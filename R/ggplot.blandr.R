  ...
  if( ciDisplay == TRUE ) {
    ...
    if( ciShading == TRUE ) {
      ba.plot <- ba.plot +
        ...
        annotate( "rect", xmin = -Inf , xmax = Inf , ymin = statistics.results$upperLOA_lowerCI , ymax = statistics.results$upperLOA_upperCI , fill="red" , alpha=0.3 ) + # Upper limits of agreement confidence interval shading
        annotate( "rect", xmin = -Inf , xmax = Inf , ymin = statistics.results$lowerLOA_lowerCI , ymax = statistics.results$lowerLOA_upperCI , fill="green" , alpha=0.3 ) # Lower limits of agreement confidence interval shading
    }
